...
import { CircleUser, House, LogOut, Search, SquarePlus, Users } from 'lucide-react';
...
const LeftColumn = () => {
    ...
    const menu = [
        ...
        { icon: CircleUser, label: 'Profile', path: `/user/${user.id}` },
    ...
    return (
        <>
            {isSmallScreen
                ? <Box sx={{ position: 'fixed', bottom: 0, display: 'flex', justifyContent: 'space-around', alignItems: 'end', width: '100%', zIndex: 99, background: 'black', paddingTop: 2.5, paddingBottom: 1.5, borderTop: '1px solid white' }}>
                    {menu.filter(item => item.path !== '/users').map((item) =>
                        <Box
                            onClick={item.path !== '/login' ? () => handleClick(item.path) : () => handleOpenLogout()}>
                            <item.icon strokeWidth={2} size={30} />
                        </Box>)}
                    {/* For Logout */}
                    <Dialog
